...
function generate_dpi_rules_form_values($form_filter = FALSE, $column = 'file')
{
  global $cache;
  $form_items = array();

  $dpifiles=dbFetchRows("select file  as 'id', file as 'title' from dpi_rules group by file");

  foreach ($dpifiles as $id => $row ){
    $device_id=$row['title'];
    $form_items[$device_id]= array('name' => $row['title'], 'group' => 'ENABLED');
    $form_items[$device_id]['group'] = 'ENABLED';
  }
  return $form_items;

}

function generate_whistler_switch_form_values($form_filter = FALSE, $column = 'file')
{
  global $cache;
  $form_items = array();

  $dpifiles=dbFetchRows("select device_id as 'id', hostname as 'title' from devices where device_id in (select device_id from vlans)");

  foreach ($dpifiles as $id => $row ){
    $device_id=$row['id'];
    $form_items[$device_id]= array('name' => $row['title'], 'group' => 'UP');
