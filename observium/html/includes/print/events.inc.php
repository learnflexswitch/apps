 ...
 * Display DPI events.
 *
 * Display pages with device/port/system DPI events on some formats.
 * Examples:
 * print_events() - display last 10 events from all devices
 * print_events(array('pagesize' => 99)) - display last 99 events from all device
 * print_events(array('pagesize' => 10, 'pageno' => 3, 'pagination' => TRUE)) - display 10 events from page 3 with pagination header
 ...
 *
 */
function print_dpi_events($vars)
{
  global $config;

  // Get events array
  $events = get_dpi_events_array($vars);
...
function get_dpi_events_array($vars)
{
  $array = array();
  ...
  $query = 'FROM `eventlog` ';
  $query .= $where . $query_permitted . generate_query_whistler_switchs();
  $query_count = 'SELECT COUNT(*) '.$query;
  $query_updated = 'SELECT MAX(`timestamp`) '.$query;
