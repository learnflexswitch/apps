...
function generate_query_whistler_switchs() {
  $devices = array_keys($permissions['device']);
  $query   = "select device_id as 'device_id' from devices where device_id in (select device_id from vlans)";
  $where= " AND device_id in (-100,-101";
  foreach (dbFetchColumn($query) as $entity_id)
  {
        $where .= ",". $entity_id;
  }
