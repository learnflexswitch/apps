...
if (preg_match('/ROS Version V(?<ros_version>\d[\w\.]+)(?<hw1>[\w\ ]+)? Software, Version (?<hw2>[\w\ \-&\(\)]+) V(?<version>\d[\w\.]+)/', $poll_device['sysDescr'], $matches))
{
  ...
  if (isset($matches['hw1']))
  {
    ...
    $hardware = end(explode(' ', $matches['hw2']));
  }
}
else if (preg_match('/Router Operating System Software [\w\ ]*ZTE Corporation(?:\ ZXROS)?\ +V(?<ros_version>\d[\w\.]+) (?<hardware>\w+) V(?<version>\d[\w\.]+)/', $poll_device['sysDescr'], $matches))
{
...
else if (preg_match('Router Operating System Software ZTE Corporation (?<hw1>[\w\ ]+)(?:\/(?<hw2>[\w\ ]+))?\ +V(?<version>\d[\w\.]+) [\w\ ]+ZXROS V(?<ros_version>\d[\w\.]+)/', $poll_device['sysDescr'], $matches))
{
...
else if (preg_match('/^ZX\w+ (?<hardware>[\w\-]+), .+? Software Version:(?: \w+)? [\w\(\)]+[vV](?<version>\d[\w\.]+)/', $poll_device['sysDescr'], $matches))
{
...
else if (preg_match('/^(?:(?<hw1>W\w+) )?(?:ZXV10_(?<hw2>W\w+)_)?V(?<version>\d[\w\.]+)/', $poll_device['sysDescr'], $matches))
{
  // ZXV10_W901_V1.0.01Z
...
else if (preg_match('/^ZXR10 (?<hardware>[\w\-]+) NOS (?<version>\d[\w\.]+)/', $poll_device['sysDescr'], $matches))
{
  // ZXR10 3904 NOS 9.05
...
else if (preg_match('/^ZX\w+ (?<hardware>[\w\-]+)/', $poll_device['sysDescr'], $matches))
{
  // All other
