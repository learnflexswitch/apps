...
if (!$os)
{
  ...
  else if (str_icontains($sysDescr, 'pfSense'))
  {
  ...
  else if (str_contains($sysDescr, 'FreeBSD') || str_starts($sysObjectId, '.1.3.6.1.4.1.8072.3.2.8'))
  {
    $os = 'freebsd';
    // But overwrite if detected more info
    if (str_starts($sysObjectId, '.1.3.6.1.4.1.12325.1.1.2.1')) // It's all BSNMP daemons
    {
      $extOutput = snmp_get($device, 'extOutput.0', '-Oqv', 'UCD-SNMP-MIB');
      if (str_icontains($extOutput, 'nas4free'))
      {
      ...
      elseif (str_icontains($extOutput, 'freenas'))
      {
    ...
    else if (str_icontains($sysDescr, 'COMPELLENT'))
    {
      // Dell Compellent
