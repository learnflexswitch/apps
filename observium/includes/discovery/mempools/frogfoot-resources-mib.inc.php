...
$free  = snmp_get($device, 'memFree.0', '-OvQ', $mib);
$total = snmp_get($device, 'memTotal.0', '-OvQ', $mib);
$used = $total - $free;

if (is_numeric($total) && is_numeric($free))
{
  discover_mempool($valid['mempool'], $device, 0, 'FROGFOOT-RESOURCES-MIB', 'Memory', 1024, $total, $used);
}
